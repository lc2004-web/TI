...
void Get_Analog_value(unsigned short *result)
{
    ...
    for(i=0;i<8;i++)
    {
        ...
        Anolag=adc_getValue(40);
				if(!Direction)result[i]=Anolag; //从1到8路，result[0]=第1路模拟值……result[7]=第8路模拟值
        else result[7-i]=Anolag; //从8到1路，result[7]=第1路模拟值………result[0]=第8路模拟值
        Anolag=0;  // 重置
    }
}
...
void normalizeAnalogValues(unsigned short *adc_value,double *Normal_factor,unsigned short *Calibrated_black,unsigned short *result,double bits)
{
    for (int i = 0; i < 8; i++) {
        unsigned short n ;
        // 计算归一化值（减去黑电平后缩放）
        if(adc_value[i]<Calibrated_black[i]) n=0;  // 如果当前值低于黑值时，输出为零
        else n = (adc_value[i] - Calibrated_black[i]) * Normal_factor[i];//先去黑，再乘归一化因子
...
unsigned char Get_Normalize_For_User(No_MCU_Sensor*sensor,unsigned short* result)
{
    ...
    {
        memcpy(result,sensor->Normal_value, 8 * sizeof(unsigned short));  // 拷贝归一化数据
        return 1;     
    }
}
...
{   
    ...
    memcpy(result,sensor->Analog_value, 8 * sizeof(unsigned short));
    if(!sensor->ok)return 0;
    else return 1;
}