...
// #include "delay.h"
/*参数	范围	说明
X 坐标	0 ~ 127	水平方向，从左到右
Y 坐标	0 ~ 63	垂直方向，从上到下
各函数的坐标/参数范围
	size1	8, 12, 16, 24	字体大小
*/
u8 OLED_GRAM[144][8];
 ...
 * @param Length 数字的位数（不包括符号位）
 * @param size1 字体大小 (8, 12, 16, 24)
 * @param mode 显示模式 (0:反色显示, 1:正常显示)
 */
 
void OLED_ShowSignedNum(u8 x, u8 y, int32_t Number, u8 Length, u8 size1, u8 mode)
{
    u8 i;
    uint32_t Number1;
    u8 char_width;
    ...
    for (i = 0; i < Length; i++)
    {
        OLED_ShowChar(x + (i + 1) * char_width, y, 
