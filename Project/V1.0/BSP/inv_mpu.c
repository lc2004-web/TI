...
/* 直接调用 bsp_siic 的 BSP_I2C_ReadReg/BSP_I2C_WriteReg, 绕过适配层 */
extern int BSP_I2C_ReadReg(uint8_t addr, uint8_t reg, uint8_t *data);
extern int BSP_I2C_WriteReg(uint8_t addr, uint8_t reg, uint8_t data);

int myi2cRead(uint8_t addr, uint8_t reg, uint8_t len, uint8_t *buf)
{
    /* 简单实现: 只支持单字节读, 与 inv_mpu 库一致 */
    if (len == 1)
        return BSP_I2C_ReadReg(addr, reg, buf);
    /* 多字节读仍走适配层 */
...
int myi2cWrite(uint8_t addr, uint8_t reg, uint8_t len, uint8_t *data)
{
    /* 简单实现: 单字节写直接调用 */
    if (len == 1)
        return BSP_I2C_WriteReg(addr, reg, *data);
    /* 多字节写仍走适配层 */
 ...
 *  @return     0 if successful.
 */
int mpu_init_step = 0;  /* 调试: 记录 mpu_init 失败位置 */
...
int mpu_init(void)
{
    ...
    mpu_init_step = 1;
    /* Reset device */
    if (BSP_I2C_WriteReg(0x68, 0x6B, 0x80))
        { mpu_init_step = 101; return -1; }
    delay_cycles(CPUCLK_FREQ / 10);  /* 100ms */

    mpu_init_step = 2;
    /* Wake up chip. */
    if (BSP_I2C_WriteReg(0x68, 0x6B, 0x00))
        { mpu_init_step = 201; return -1; }

#if defined MPU6050
    /* Check product revision — 适配 MPU6500 (WHO_AM_I=0x70) */
    /* 直接用单字节读 prod_id, 跳过 accel_offs 多字节读 (MPU6500 不支持) */
    {
        uint8_t prod_id_val;
        mpu_init_step = 3;
        if (BSP_I2C_ReadReg(st.hw->addr, st.reg->prod_id, &prod_id_val))
            return -1;
        rev = prod_id_val & 0x0F;
        if (!rev) {
            /* MPU6500 returns 0 at prod_id; treat as compatible */
...
#define MPU6500_MEM_REV_ADDR    (0x17)
    /* Fix: mpu_read_mem requires non-zero sensors; init early */
