...
{
    return (yaw_settle_cnt >= YAW_SETTLE_TARGET) ? 1 : 0;
}

int IMU_GetSettleCnt(void)
{
    return yaw_settle_cnt;
}
...
int IMU_Read(IMU_Attitude_t *att)
{
    ...
    /* 等待 DMP 稳定后记录 Yaw 零点 (约 2 秒) */
    if (yaw_settle_cnt < YAW_SETTLE_TARGET) {
        yaw_settle_cnt++;
        if (yaw_settle_cnt == YAW_SETTLE_TARGET) {
            yaw_offset = att->yaw;  /* 记录此刻的 Yaw 作为零点 */
        }
    }
