...
/**
 * @brief  初始化舵机 PWM 输出, 直接设置到指定角度 (避免跳变)
 * @param  initial_angle  初始角度 [0, 180]
 */
void SERVO_Init(uint8_t initial_angle)
{
    ...
    /* 6. 直接输出目标角度, 无中间跳变 */
    cmp = SERVO_CMP_0DEG - (uint16_t)(((uint32_t)initial_angle * SERVO_CMP_RANGE) / 180U);
